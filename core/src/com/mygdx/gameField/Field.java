			...
			for (int arrayPosY = 0; arrayPosY < cells[arrayPosX].length; arrayPosY++) {
				...
				for (int i = +1; i >= -1; i--) {
					for (int k = -1; k <= +1; k++) {
						...
						if (GameUtils.isIn2DArrayBound(new Coordinates(arrayPosX + k, arrayPosY + i), cells.length,
								cells[arrayPosX].length)) {

							cells[arrayPosX][arrayPosY].getNearCells()
									.getNearCellsArray()[counter] = cells[arrayPosX + k][arrayPosY + i];
