...
public class GameEndlessMode extends GameModeState {
	...
	public void create() {
		...
		acceptAddNewNameSides = new Coordinates(bigSquareSides.getCoordinateX() / 2 - 2 * sigmaSpace,
				bigSquareSides.getCoordinateY() / 6);
		declineAddNewNameSides = acceptAddNewNameSides;

		fieldForNameSides = addInfoRequestSides;

		nextGameBar = new BarWithTextCollection(BarWithTextCollectionParameters.getParameters("nextBar",
				BarWithText.newBarWithText(FloatCoordinates.newCoordinates(rectangleWidth, rectangleHeight),
						FloatCoordinates.newCoordinates(screenWidth / 2, screenHeight - spriteSize / 2), "PROXIMO",
						Color.GRAY, Color.WHITE)));

		pointsBar = new BarWithTextCollection(BarWithTextCollectionParameters.getParameters("pointsBar",
				BarWithText.newBarWithText(FloatCoordinates.newCoordinates(pointsRectangleWidth, pointsRectangleHeight),
						FloatCoordinates.newCoordinates(pointsRectangleWidth / 2, spriteSize / 2),
						"PONTOS : " + gamePoints, Color.DARK_GRAY, Color.WHITE))

		);

		nameSpaceBar = new BarWithTextCollection(
				BarWithTextCollectionParameters.getParameters("bigSquareBar",
						BarWithText.newBarWithText(FloatCoordinates.newCoordinates(bigSquareSides),
								FloatCoordinates.newCoordinates(screenWidth / 2, screenHeight / 2), "", Color.DARK_GRAY,
								Color.WHITE)),

				BarWithTextCollectionParameters.getParameters("addRequest",
						BarWithText.newBarWithText(FloatCoordinates.newCoordinates(addInfoRequestSides),
								FloatCoordinates.newCoordinates(screenWidth / 2,
										screenHeight / 2 + bigSquareSides.getCoordinateY() * 1 / 2
												- addInfoRequestSides.getCoordinateY() * 1 / 2),
								"Adicionar nome", Color.DARK_GRAY, Color.WHITE)),

				BarWithTextCollectionParameters.getParameters("add",
						BarWithText.newBarWithText(FloatCoordinates.newCoordinates(acceptAddNewNameSides),
								FloatCoordinates.newCoordinates(
										screenWidth / 2 - bigSquareSides.getCoordinateX() / 2
												+ acceptAddNewNameSides.getCoordinateX() / 2 + sigmaSpace,
										screenHeight / 2 - bigSquareSides.getCoordinateY() / 2
												+ acceptAddNewNameSides.getCoordinateY() / 2 + 3 * sigmaSpace),
								"FEITO", Color.GRAY, Color.WHITE)),

				BarWithTextCollectionParameters.getParameters("decline",
						BarWithText.newBarWithText(FloatCoordinates.newCoordinates(declineAddNewNameSides),
								FloatCoordinates.newCoordinates(
										screenWidth / 2 + bigSquareSides.getCoordinateX() / 2
												- declineAddNewNameSides.getCoordinateX() / 2 - sigmaSpace,
										screenHeight / 2 - bigSquareSides.getCoordinateY() / 2
												+ declineAddNewNameSides.getCoordinateY() / 2 + 3 * sigmaSpace),
								"NEGAR", Color.GRAY, Color.WHITE)
		...
		nameBar = new WritableBar(
				FloatCoordinates.newCoordinates(fieldForNameSides.getCoordinateX() / 2,
						fieldForNameSides.getCoordinateY()),
				FloatCoordinates.newCoordinates(screenWidth / 2, screenHeight / 2), Color.WHITE, Color.WHITE,
